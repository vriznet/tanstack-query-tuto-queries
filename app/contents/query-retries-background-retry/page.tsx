...
export default function QueryRetries() {
  // When using refetchInterval with refetchIntervalInBackground: true,
  // retries will pause when the browser tab is inactive.
  // This happens because retries respect the same focus behavior as regular refetches.
  const { status, data, error, failureReason } = useQuery({
    queryKey: ["todos"],
    queryFn: fetchTodos,
